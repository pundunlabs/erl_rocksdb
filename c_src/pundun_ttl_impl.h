...
class PundunTTLImpl : public PundunTTL {
 public:
  static void SanitizeOptions(int32_t ttl, ColumnFamilyOptions* options,
                              Env* env);

  explicit PundunTTLImpl(DB* db);

  virtual ~PundunTTLImpl();
...
class PundunTtlIterator : public Iterator {

 public:
  explicit PundunTtlIterator(Iterator* iter) : iter_(iter) { assert(iter_); }

  ~PundunTtlIterator() { delete iter_; }
  ...
  int32_t timestamp() const {
    return DecodeFixed32(iter_->value().data() + iter_->value().size() -
                         PundunTTLImpl::kTSLength);
  }

  Slice value() const override {
    // TODO: handle timestamp corruption like in general iterator semantics
    assert(PundunTTLImpl::SanityCheckTimestamp(iter_->value()).ok());
    Slice trimmed_value = iter_->value();
    trimmed_value.size_ -= PundunTTLImpl::kTSLength;
...
class PundunTtlCompactionFilter : public CompactionFilter {
 public:
  PundunTtlCompactionFilter(
  ...
  virtual bool Filter(int level, const Slice& key, const Slice& old_val,
      ...
      override {
    if (PundunTTLImpl::IsStale(old_val, ttl_, env_)) {
      return true;
    }
    if (user_comp_filter_ == nullptr) {
      return false;
    }
    assert(old_val.size() >= PundunTTLImpl::kTSLength);
    Slice old_val_without_ts(old_val.data(),
                             old_val.size() - PundunTTLImpl::kTSLength);
    ...
    if (*value_changed) {
      new_val->append(
          old_val.data() + old_val.size() - PundunTTLImpl::kTSLength,
          PundunTTLImpl::kTSLength);
    }
    return false;
  }
...
class PundunTtlCompactionFilterFactory : public CompactionFilterFactory {
 public:
  PundunTtlCompactionFilterFactory(
      int32_t ttl, Env* env,
  ...
  virtual std::unique_ptr<CompactionFilter> CreateCompactionFilter(
    ...
    return std::unique_ptr<PundunTtlCompactionFilter>(new PundunTtlCompactionFilter(
        ttl_, env_, nullptr, std::move(user_comp_filter_from_factory)));
  }

  virtual const char* Name() const override {
    return "PundunTtlCompactionFilterFactory";
  }
...
class PundunTtlMergeOperator : public MergeOperator {

 public:
  explicit PundunTtlMergeOperator(const std::shared_ptr<MergeOperator>& merge_op,
                            Env* env)
  ...
  virtual bool FullMergeV2(const MergeOperationInput& merge_in,
    ...
    const uint32_t ts_len = PundunTTLImpl::kTSLength;
